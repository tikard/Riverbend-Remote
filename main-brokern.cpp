...
void remoteRequestSend(String cmd, int wellID){
  if (cmd.equals("STATE")){
    requestState(String(wellID));
    return;
  }

  if (cmd.equals("STATUS")){
    requestStatus(String(wellID));
    return;
  }

  if (cmd.equals("ERRORS")){
    requestErrors(String(wellID));
    return;
  }

  if (cmd.equals("HEARTBEAT")){
    requestHeartBeat(String(wellID));
    return;
  }
}
...
void setup(){
  ...
  */
  ...
  initWebSocket();   // **** Start up WebSockets must be after webserver start 
...
void loop() {
  ...
  if(receiveflag){
    ...
    processMsg(lastpacket);  // Process/discard the message
    receiveflag = false;
    delay(5);
    displaySendReceive();
  }
